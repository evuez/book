...
This line will create a new binding named `foo` and bind it to the value `bar`.
In many languages, `foo` is called a *variable*, but Rust’s variable bindings
have a few differences. For example, they’re immutable by default. To make your
...
Now you know that `let mut guess` will introduce a mutable binding named
...
to know is that like `let` bindings, references are immutable by default.
...
forcing you to create two unique bindings, like `guess_str` and `guess` for
...
colon (`:`) after `guess` tells Rust you’ll annotate the binding’s type. Rust
